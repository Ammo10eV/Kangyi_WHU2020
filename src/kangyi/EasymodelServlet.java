...
//	 	
	 		...
	 		else if(pagename.equals("China")) {
	 			args1=new String[]{"python","C:\\Users\\Thinkpad\\eclipse-workspace\\kangyi\\src\\resources\\Chinamodel_forweb.py",time1,choose1,time2,choose2,time3,choose3};
	 		   
	 		}
	 		else if(pagename.equals("Italy")) {
	 			args1=new String[]{"python","C:\\Users\\Thinkpad\\eclipse-workspace\\kangyi\\src\\resources\\Italymodel_forweb.py",time1,choose1,time2,choose2,time3,choose3};
	 		   
	 		}
	 		else if(pagename.equals("Spain")) {
	 			args1=new String[]{"python","C:\\Users\\Thinkpad\\eclipse-workspace\\kangyi\\src\\resources\\Spainmodel_forweb.py",time1,choose1,time2,choose2,time3,choose3};
...
//	         request.getRequestDispatcher("info-model-result.jsp").forward(request, response);
	         if(pagename.equals("Korean")) {
	        	//获取累计确诊数量——韩国
		         ...
		         String[] results=dF_forDiag.getAccumulate("korean");
		 		...
		 		//获取累计确诊数量——中国
		         dataFunction dF_forDiag=new dataFunction();
		         String[] results=dF_forDiag.getAccumulate("china");
		         request.setAttribute("Cum_diag",results[0]);
		         request.setAttribute("Add_diag",results[1]);
		 		request.getRequestDispatcher("predict-easy-China-result.jsp").forward(request, response);
			    
		 	  }
		 	else if(pagename.equals("Italy")) {
		 		//获取累计确诊数量——意大利
		         dataFunction dF_forDiag=new dataFunction();
		         String[] results=dF_forDiag.getAccumulate("italy");
		         request.setAttribute("Cum_diag",results[0]);
		         request.setAttribute("Add_diag",results[1]);
		         
		 		request.getRequestDispatcher("predict-easy-Italy-result.jsp").forward(request, response);
			    
		 	  }
		 	else if(pagename.equals("Spain")) {
		 		//获取累计确诊数量——西班牙
		         dataFunction dF_forDiag=new dataFunction();
		         String[] results=dF_forDiag.getAccumulate("spain");
		         request.setAttribute("Cum_diag",results[0]);
		         request.setAttribute("Add_diag",results[1]);
		 		
		 		request.getRequestDispatcher("predict-easy-Spain-result.jsp").forward(request, response);
